...
Call log:
  - waiting for navigation until "load"
    - navigated to "https://uat-travel-idfc.travel-loyalty.com/flight/booking/7758"
...
```ts
  86  |         const text = await selector.textContent();
  87  |         if (!text || text.trim() === "") {
  88  |             await expect.soft(selector).toBeEmpty();
  89  |         } else {
  90  |             console.error(`Container is not empty for locator: ${locator}`);
  91  |         }
  92  |     }
