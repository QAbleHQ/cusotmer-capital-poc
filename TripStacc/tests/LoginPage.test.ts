...
import tcTestData from '../../testdata/tripStacc.json';
...
test('SC_001, SC_002: Verify user can login with valid credentials and Home page with multiple card and without multiple cards selection', { tag: ['@IDFC','@BOB', '@Loginpage','@Smoke','@Sanity'] }, async () => {
  await test.step('Step 1: Enter valid mobile number', async () => {
    await LoginPage.verifyMobileNumberFieldAcceptsInput(page, tcTestData.loginDataFill.mobileNumber);
  });
  ...
  await test.step('Step 3: Enter valid OTP and verify OTP field accepts input', async () => {
    await LoginPage.verifyOtpFieldAcceptsInput(page, tcTestData.loginDataFill.otp);
  });
